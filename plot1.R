...
# data <- read.table("~/Downloads/household_power_consumption.txt", header=TRUE, sep=";", stringsAsFactors=F, colClasses=c("myDate", "myTime", rep("double", 7)), na.strings = "?", nrows = 5)
data <- read.table("~/Downloads/household_power_consumption.txt", header=TRUE, sep=";", stringsAsFactors=F, colClasses=c("myDate", "myTime", rep("double", 7)), na.strings = "?")
